...
for j = 1:N_x
    ...
    if N >= 4
        L(N-1,:) = 0; L(N-1,N-3) = ccoef; L(N-1,N-2) = bcoef; L(N-1,N-1) = acoef + ccoef; L(N-1,N) = bcoef;
    end
...
delta_x = x(2)-x(1);
[psi_z, psi_x] = gradient(psi, delta_z, delta_x);

% Build interpolants for psi derivatives
F_psiz = scatteredInterpolant(X(:), Z(:), psi_z(:), 'linear', 'nearest');
F_psix = scatteredInterpolant(X(:), Z(:), psi_x(:), 'linear', 'nearest');
...
% Diagnostics: quick stats to check magnitudes and signs
%fprintf('psi: max=%g, min=%g, range=%g\n', max(psi(:)), min(psi(:)), max(psi(:))-min(psi(:)));
%fprintf('psi_z: max=%g, min=%g\n', max(psi_z(:)), min(psi_z(:)));
%fprintf('psi_x: max=%g, min=%g\n', max(psi_x(:)), min(psi_x(:)));
%fprintf('max|psi_hat|=%g\n', max(abs(psi_hat(:))));

% Shorter test run and finer timestep for readable trajectories
t_start = 0; t_end = 2000; delta_t = 1;

% Plot psi contours and velocity quiver (subsampled)
figure;
...
xseed = 0;
for zseed = 40:40:800
    [x_traj,z_traj] = plotXplotY(xseed, zseed, t_end, delta_t, F_psiz, F_psix, U0, L_x, L_z);
    plot(x_traj, z_traj, 'LineWidth', 1.2);
end

function [X_trajectory,Z_trajectory] = plotXplotY(X0,Z0,t_end,delta_t,F_psiz,F_psix,U0,L_x,L_z)
    t_start = 0;
    ...
    for i = 1:N
        ...
        psi_z_i = F_psiz(X_current, Z_current);
        psi_x_i = F_psix(X_current, Z_current);
        X_star_new = X_current + (U0 + psi_z_i) * delta_t;
        Z_star_new = Z_current - (psi_x_i) * delta_t;
        psi_z_s = F_psiz(X_star_new, Z_star_new);
        psi_x_s = F_psix(X_star_new, Z_star_new);
        X_new = X_current + delta_t * (U0 + 0.5*(psi_z_i + psi_z_s));
        Z_new = Z_current - delta_t * (0.5*(psi_x_i + psi_x_s));
